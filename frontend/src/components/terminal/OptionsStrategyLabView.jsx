  ...
  return (
    <div style={{ padding: 'clamp(14px, 3vw, 24px)', display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14 }}>
        <div style={{ background: '#0C1022', border: '1px solid rgba(99,102,241,0.2)', padding: '16px 18px', borderRadius: 12 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', textTransform: 'uppercase' }}>UNDERLYING PRICE</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#F0F0FF', fontFamily: 'JetBrains Mono, monospace' }}>
            ₹{data?.underlying_price}
          </div>
        </div>

        <div style={{ background: '#0C1022', border: '1px solid rgba(16,185,129,0.3)', padding: '16px 18px', borderRadius: 12 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', textTransform: 'uppercase' }}>MAX PROFIT</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#10B981', fontFamily: 'JetBrains Mono, monospace' }}>
            {typeof data?.max_profit === 'number' ? `+₹${data.max_profit}` : data?.max_profit}
          </div>
        </div>

        <div style={{ background: '#0C1022', border: '1px solid rgba(239,83,80,0.3)', padding: '16px 18px', borderRadius: 12 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', textTransform: 'uppercase' }}>MAX LOSS</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#EF5350', fontFamily: 'JetBrains Mono, monospace' }}>
            {data?.max_loss} ₹
          </div>
        </div>

        <div style={{ background: '#0C1022', border: '1px solid rgba(99,102,241,0.2)', padding: '16px 18px', borderRadius: 12 }}>
          <div style={{ fontSize: '0.7rem', color: '#6B7280', textTransform: 'uppercase' }}>BREAKEVEN STRIKES</div>
          <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#38BDF8', fontFamily: 'JetBrains Mono, monospace' }}>
