...
function Menu({ children, align = 'left', style }) {
  return <div style={{ position: 'absolute', top: 'calc(100% + 7px)', [align]: 0, minWidth: 190, padding: 6, border: '1px solid rgba(148,163,184,0.18)', borderRadius: 8, background: '#111827', boxShadow: '0 18px 40px rgba(0,0,0,0.45)', zIndex: 120, ...(style || {}) }}>{children}</div>;
}
...
export default function ChartToolbar({
  ...
  return <div ref={rootRef} style={{ flexShrink: 0, position: 'relative', zIndex: 40, display: 'flex', alignItems: 'center', flexWrap: 'nowrap', gap: 5, height: 36, minHeight: 36, padding: isMobile ? '0 6px' : '0 8px', overflow: 'visible', background: '#0B0F1C', border: '1px solid rgba(148,163,184,0.14)', borderRadius: 7, color: '#CBD5E1', fontFamily: 'Inter, ui-sans-serif, sans-serif' }}>
    ...
    <div style={{ display: 'contents' }}>
      ...
      <Group><div style={{ position: 'relative' }}><button type="button" onClick={() => setOpenMenu(openMenu === 'draw' ? null : 'draw')} title={drawToolLabel} style={{ ...compactButton, color: showDrawingTools ? '#7DD3FC' : '#CBD5E1' }}><PenTool size={14} />{!isMobile && <span style={{ maxWidth: 110, overflow: 'hidden', textOverflow: 'ellipsis' }}>{drawToolLabel}</span>}<ChevronDown size={12} /></button>{openMenu === 'draw' && <Menu style={{ minWidth: 230, maxHeight: 340, overflowY: 'auto' }}><MenuItem active={showDrawingTools} onClick={() => { onToggleDrawingTools(); }}>Drawing rail</MenuItem>{DRAWING_TOOL_GROUPS.map(group => <div key={group.id}><div style={drawGroupHeader}>{group.label.toUpperCase()}</div>{group.tools.map(tool => <MenuItem key={tool.id} active={activeDrawingTool === tool.id} onClick={() => { onSelectDrawingTool(tool.id); setOpenMenu(null); }}>{tool.label}</MenuItem>)}</div>)}</Menu>}</div></Group>
