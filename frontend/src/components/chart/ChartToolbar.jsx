...
export default function ChartToolbar({
  ...
  return <div ref={rootRef} style={{ flexShrink: 0, position: 'relative', zIndex: 70, display: 'flex', alignItems: 'center', flexWrap: 'nowrap', gap: 5, height: 36, minHeight: 36, padding: isMobile ? '0 6px' : '0 8px', overflow: 'visible', background: tk.toolbarBg, border: `1px solid ${tk.toolbarBorder}`, borderRadius: 7, color: tk.toolbarText, fontFamily: 'Inter, ui-sans-serif, sans-serif' }}>
    ...
    <div style={{ display: 'contents' }}>
      ...
      {!(isMobile || isTablet) && <Group><span style={sectionLabel}>SCALE</span>{['normal', 'log', 'percentage'].map(mode => <button key={mode} type="button" onClick={() => onPriceScaleModeChange(mode)} title={`${mode === 'normal' ? 'Auto' : mode === 'percentage' ? 'Percentage' : 'Logarithmic'} scale`} style={{ ...chipButton, background: priceScaleMode === mode ? 'rgba(2,132,199,0.14)' : 'transparent', color: priceScaleMode === mode ? tk.toolbarActive : tk.toolbarMuted, fontWeight: 700 }}>{mode === 'normal' ? 'Auto' : mode === 'percentage' ? '%' : 'Log'}</button>)}</Group>}
      {!isTablet && <button type="button" onClick={onToggleVolume} title={showVolume ? 'Hide volume panel' : 'Show volume panel'} aria-label={showVolume ? 'Hide volume panel' : 'Show volume panel'} style={{ ...compactButton, color: showVolume ? tk.toolbarActive : tk.toolbarMuted, background: showVolume ? 'rgba(56,189,248,0.14)' : 'transparent', fontWeight: showVolume ? 700 : 400 }}><BarChart2 size={14} />{!isMobile && 'Volume'}</button>}
      {!isTablet && <button type="button" onClick={onToggleReplay} title="Bar Replay — step through history bar by bar (Alt+R)" aria-label="Bar Replay" style={{ ...compactButton, color: isReplaying ? '#EF5350' : tk.toolbarMuted, background: isReplaying ? 'rgba(239,83,80,0.14)' : 'transparent', fontWeight: isReplaying ? 700 : 400 }}><History size={14} />{!isMobile && 'Replay'}</button>}
      {!isTablet && <button type="button" onClick={onToggleTradeBar} title="Live Paper Trade Bar on Chart" aria-label="Paper Trade Bar" style={{ ...compactButton, color: showTradeBar ? '#10B981' : tk.toolbarMuted, background: showTradeBar ? 'rgba(16,185,129,0.14)' : 'transparent', fontWeight: showTradeBar ? 800 : 400 }}><Zap size={14} />{!isMobile && 'Trade'}</button>}
      {!isTablet && <button type="button" onClick={onToggleTradeDocket} title="Paper Positions & Trade History Panel" aria-label="Paper Positions Panel" style={{ ...compactButton, color: showTradeDocket ? '#818CF8' : tk.toolbarMuted, background: showTradeDocket ? 'rgba(99,102,241,0.14)' : 'transparent', fontWeight: showTradeDocket ? 800 : 400 }}><Wallet size={14} />{!isMobile && 'Panel'}{paperPositionCount > 0 && <span style={{ ...countBadge, background: '#10B981', color: '#FFFFFF' }}>{paperPositionCount}</span>}</button>}
      {(isMobile || isTablet) && <button type="button" onClick={() => setOpenMenu(openMenu === 'more' ? null : 'more')} title="More chart controls" aria-label="More chart controls" style={{ ...themedIconButton(tk), marginLeft: 'auto' }}><SlidersHorizontal size={15} /></button>}
      {openMenu === 'more' && <Menu align="right"><MenuItem onClick={onOpenIndicators} icon={<Activity size={14} />}>Indicators {activeIndicatorCount > 0 && `(${activeIndicatorCount})`}</MenuItem><MenuItem onClick={onToggleVolume} icon={<BarChart2 size={14} />}>{showVolume ? 'Hide Volume Panel' : 'Show Volume Panel'}</MenuItem><MenuItem onClick={onToggleReplay} icon={<History size={14} />}>{isReplaying ? 'Exit Bar Replay' : 'Bar Replay'}</MenuItem><MenuItem onClick={() => onPriceScaleModeChange(priceScaleMode === 'normal' ? 'log' : priceScaleMode === 'log' ? 'percentage' : 'normal')} icon={<BarChart2 size={14} />}>Scale: {priceScaleMode === 'normal' ? 'Auto' : priceScaleMode === 'log' ? 'Log' : '%'}</MenuItem><MenuItem onClick={onToggleTradeBar} icon={<Zap size={14} />}>Toggle Trade Bar</MenuItem><MenuItem onClick={onToggleTradeDocket} icon={<Wallet size={14} />}>Paper Trading Panel</MenuItem><MenuItem onClick={onOpenSettings} icon={<Settings size={14} />}>Chart Settings</MenuItem><MenuItem onClick={onResetZoom} icon={<RotateCcw size={14} />}>Focus latest candles</MenuItem></Menu>}
    </div>
  </div>;
}
